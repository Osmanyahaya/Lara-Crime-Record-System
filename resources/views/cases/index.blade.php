    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{route('cases.create')}}"
                class="px-4 py-2 hover:text-white text-white rounded-md text-base bg-gradient-to-r from-green-400 to-blue-500 hover:from-blue-500 hover:bg-black">Add New Case
            </a>
            @endif
        </div>
    </x-slot>
                ...
                <div class="">
                    <table class="w-full table-auto mb-4">
                        <thead class="bg-blue-700 text-white">
                            <tr>
                                <th class="border px-4 py-4">Case Number</th>
                        ...
                        <tbody>
                            @forelse ($cases as $case)
                                <tr class="">
                                    <td class="border px-4 py-2">{{ $case->reference }}</td>
                                    ...
                                    <td class="border px-4 py-2">
                                        <div class="flex items-center justify-center">
                                             @if(Auth::user()->hasRole('admin') || Auth::user()->hasRole('nco'))
                                            <a href="{{route('cases.edit',$case->id)}}" class="mx-1 hover:bg-black hover:text-white duration-200 leading-none bg-blue-700 text-white px-3 py-2 text-sm rounded-md">Edit</a>
                                            @endif
                                            ...
                                            <form action="{{route('cases.destroy',$case->id)}}" method="POST" onsubmit="return confirm('Are you sure you want to delete Case?')">
                                                @csrf
                                                @method('DELETE')
